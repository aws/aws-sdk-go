...
// object.
func SignAwsRequest(req *request.Request) {
	...
	req.Error = err
	req.Time = signer.Time
	req.SignedHeaderVals = signer.signedHeaderVals
}

// Sign raw net/http requests with signature version 4.
//
// Will sign the request with the paased in credentials object.
// Other settings are also needed to meet SigV4's requirements such as:
// region: The aws region, eg us-west-2
// name: the name of the service being called, eg dyanamodb.
// (name and region come from the URL, eg https://dyanmodb.us-west-2.amazonaws.com)
// time: the current type, typically time.Now().
// expireTime: for presign request how long we want the url to be valid for, if not presign, then 0
// logger: any aws logger
// debug: a log level
// notHoist: ?
...
func (v4 *Signer) Sign() error {
	...
	if v4.isRequestSigned() {
		...
		if v4.isPresign {
			...
			v4.Request.URL.RawQuery = v4.Request.Query.Encode()
		}
	}
	...
	if v4.isPresign {
		v4.Request.Query.Set("X-Amz-Algorithm", authHeaderPrefix)
		if v4.CredValues.SessionToken != "" {
			v4.Request.Query.Set("X-Amz-Security-Token", v4.CredValues.SessionToken)
		} else {
			v4.Request.Query.Del("X-Amz-Security-Token")
		}
	...
	if v4.isPresign {
		if !v4.notHoist {
			...
			for k := range urlValues {
				v4.Request.Query[k] = urlValues[k]
			}
		}
	}
