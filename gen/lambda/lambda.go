...
// AddEventSource identifies a stream as an event source for an AWS Lambda
// function. It can be either an Amazon Kinesis stream or a Amazon DynamoDB
// stream. AWS Lambda invokes the specified function when records are
// posted to the stream. This is the pull model, where AWS Lambda invokes
// the function. For more information, go to AWS Lambda: How it Works in
// the AWS Lambda Developer Guide. This association between an Amazon
// Kinesis stream and an AWS Lambda function is called the event source
// mapping. You provide the configuration information (for example, which
// stream to read from and which AWS Lambda function to invoke) for the
// event source mapping in the request body. Each event source, such as a
// Kinesis stream, can only be associated with one AWS Lambda function. If
// you call AddEventSource for an event source that is already mapped to
// another AWS Lambda function, the existing mapping is updated to call the
// new function instead of the old one. This operation requires permission
// for the iam:PassRole action for the IAM role. It also requires
...
// ListEventSources returns a list of event source mappings you created
// using the AddEventSource (see AddEventSource ), where you identify a
// stream as event source. This list does not include Amazon S3 event
// sources. For each mapping, the API returns configuration information.
...
type EventSourceConfiguration struct {
	...
	LastModified aws.StringValue   `json:"LastModified,omitempty"`
	Parameters   map[string]string `json:"Parameters,omitempty"`
