...
// Zonal autoshift is a capability in Route 53 ARC where you authorize Amazon
...
// must also configure practice runs when you enable zonal autoshift for a resource.
// Practice runs start weekly zonal shifts for a resource, to shift traffic
// for the resource away from an Availability Zone. Practice runs help you to
// make sure, on a regular basis, that you have enough capacity in all the Availability
// Zones in an Amazon Web Services Region for your application to continue to
// operate normally when traffic for a resource is shifted away from one Availability
// Zone.
//
// Before you configure practice runs or enable zonal autoshift, we strongly
// recommend that you prescale your application resource capacity in all Availability
// Zones in the Region where your application resources are deployed. You should
// not rely on scaling on demand when an autoshift or practice run starts. Zonal
// autoshift, including practice runs, works independently, and does not wait
// for auto scaling actions to complete. Relying on auto scaling, instead of
...
// Amazon Web Services only starts an autoshift when Amazon Web Services telemetry
// detects that there is an Availability Zone impairment that could potentially
// impact customers. In some cases, resources might be shifted away that are
// not experiencing impact.
//
