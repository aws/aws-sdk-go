...
// a standard HTTP API and an AWS EventBridge event stream for responses. JSON
...
//
// You generate a signature using valid AWS credentials that have permission
// to perform the requested action. For example, you must sign PutMetadata requests
// with a signature generated from an IAM user account that has the ivs:PutMetadata
// permission.
//
...
//    to use both filters, you will get an error (409 Conflict Exception).
//
...
//    with a maximum 1 KB payload. (If 5 TPS is not sufficient for your needs,
//    we recommend batching your data into a single PutMetadata call.)
//
