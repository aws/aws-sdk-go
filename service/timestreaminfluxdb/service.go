...
// TimestreamInfluxDB provides the API operation methods for making requests to
// Timestream InfluxDB. See this package's package overview docs
// for details on the service.
//
// TimestreamInfluxDB methods are safe to use concurrently. It is not safe to
...
func New(p client.ConfigProvider, cfgs ...*aws.Config) *TimestreamInfluxDB {
	...
	if c.SigningNameDerived || len(c.SigningName) == 0 {
		c.SigningName = "timestream-influxdb"
	}
	return newClient(*c.Config, c.Handlers, c.PartitionID, c.Endpoint, c.SigningRegion, c.SigningName, c.ResolvedRegion)
}

// newClient creates, initializes and returns a new service client instance.
func newClient(cfg aws.Config, handlers request.Handlers, partitionID, endpoint, signingRegion, signingName, resolvedRegion string) *TimestreamInfluxDB {
	svc := &TimestreamInfluxDB{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a TimestreamInfluxDB operation and runs any
// custom request initialization.
func (c *TimestreamInfluxDB) newRequest(op *request.Operation, params, data interface{}) *request.Request {
