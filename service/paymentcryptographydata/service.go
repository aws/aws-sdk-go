...
// PaymentCryptographyData provides the API operation methods for making requests to
// Payment Cryptography Data Plane. See this package's package overview docs
// for details on the service.
//
// PaymentCryptographyData methods are safe to use concurrently. It is not safe to
...
func New(p client.ConfigProvider, cfgs ...*aws.Config) *PaymentCryptographyData {
	...
	if c.SigningNameDerived || len(c.SigningName) == 0 {
		c.SigningName = "payment-cryptography"
	}
	return newClient(*c.Config, c.Handlers, c.PartitionID, c.Endpoint, c.SigningRegion, c.SigningName, c.ResolvedRegion)
}

// newClient creates, initializes and returns a new service client instance.
func newClient(cfg aws.Config, handlers request.Handlers, partitionID, endpoint, signingRegion, signingName, resolvedRegion string) *PaymentCryptographyData {
	svc := &PaymentCryptographyData{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a PaymentCryptographyData operation and runs any
// custom request initialization.
func (c *PaymentCryptographyData) newRequest(op *request.Operation, params, data interface{}) *request.Request {
