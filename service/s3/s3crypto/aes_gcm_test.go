...
func aesgcmTest(t *testing.T, iv, key, plaintext, expected, tag []byte) {
	gcm, err := NewAESGCM(key, iv)
	assert.Nil(t, err)
	cipherdata, err := gcm.Encrypt(bytes.NewReader(plaintext))
	assert.Nil(t, err)
